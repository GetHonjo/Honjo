...
namespace HonjoLib
{
    public class Honjo
    {
        ...
        public string Compile(string s, object o, bool trimEveryResult = false)
        {
            ...
            var iterationRegex = new Regex(@"\{{item(.*?) in (.*?)}}(.*?)\{{/item}}",
            ...
            var matchIfElseRegex = new Regex(@"\{{if (.*?)(==|>|<|>=|<=|!=|&&)(.*?)}}(.*?){{else}}(.*?)\{{/if}}",
                RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.Singleline);
            var matchIfRegex = new Regex(@"\{{if (.*?)(==|>|<|>=|<=|!=|&&)(.*?)}}(.*?)\{{/if}}",
                RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.Singleline);

            var matchNestedIfElseRegex =
                new Regex(@"\{{if (.*?)(==|>|<|>=|<=|!=|&&)(.*?)}}(.*?){{else}}(.*?)\{{/if}}(.*?){{/if}}",
                    RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.Singleline);

            s = matchNestedIfElseRegex.Replace(s,
                m =>
                {
                    ...
                    var codeString4 = Compile(nest, o, trimEveryResult);
        ...
        internal Dictionary<string, Tuple<dynamic, Type>> ToDictionary<T>(T myVar, string root = "")
        {
            if (myVar == null)
            {
